...
void HPPImage::create_vk_image(vkb::core::device &device, vk::ImageViewType image_view_type, vk::ImageCreateFlags flags)
{
	assert(!vk_image && !vk_image_view && "Vulkan HPPImage already constructed");

	vk_image = std::make_unique<vkb::core::image>(device,
	                                                 get_extent(),
	                                                 format,
	...
	vk_image_view = std::make_unique<vkb::core::image_view>(*vk_image, image_view_type);
...
const vkb::core::image &HPPImage::get_vk_image() const
{
	assert(vk_image && "Vulkan HPPImage was not created");
	return *vk_image;
}

const vkb::core::image_view &HPPImage::get_vk_image_view() const
{
