    ...
    sample{sample_}, content_scale_factor{window.get_content_scale_factor()}, dpi_factor{window.get_dpi_factor() * content_scale_factor}, explicit_update{explicit_update}, stats_view(stats)
{
	...
	font_image = std::make_unique<vkb::core::image>(device, font_extent, vk::Format::eR8G8B8A8Unorm,
	                                                   vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eTransferDst,
	                                                   VMA_MEMORY_USAGE_GPU_ONLY);
	font_image->set_debug_name("GUI font image");

	font_image_view = std::make_unique<vkb::core::image_view>(*font_image, vk::ImageViewType::e2D);
	...
	// Upload font data into the vulkan image memory
	{
		...
		vkb::fence_pool fence_pool(device);
		...
		buffer_copy_region.imageSubresource.layerCount = font_image_view->subresource_range().layerCount;
		buffer_copy_region.imageSubresource.aspectMask = font_image_view->subresource_range().aspectMask;
		buffer_copy_region.imageExtent                 = font_image->extent();
	...
	vkb::core::shader_source vert_shader("imgui.vert");
	vkb::core::shader_source frag_shader("imgui.frag");

	std::vector<vkb::core::shader_module *> shader_modules;
...
void HPPGui::draw(vkb::core::command_buffer &command_buffer)
{
	...
	vkb::core::scoped_debug_label debug_label(command_buffer, "GUI");
