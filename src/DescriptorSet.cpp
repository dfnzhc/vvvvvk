...
vk_descriptor_set::vk_descriptor_set(vk_device& device,
...
void vk_descriptor_set::reset(const BindingMap<VkDescriptorBufferInfo>& new_buffer_infos,
...
void vk_descriptor_set::prepare()
{
    ...
    for (auto& binding_it: buffer_infos) {
        ...
        if (auto binding_info = descriptor_set_layout.get_layout_binding(binding_index)) {
            ...
            for (auto& element_it: buffer_bindings) {
                ...
                size_t uniform_buffer_range_limit = device.get_gpu().properties().limits.maxUniformBufferRange;
                size_t storage_buffer_range_limit = device.get_gpu().properties().limits.maxStorageBufferRange;
...
void vk_descriptor_set::update(const std::vector<uint32_t>& bindings_to_update)
{
...
void vk_descriptor_set::apply_writes() const
{
    vkUpdateDescriptorSets(device.handle(),
...
VkDescriptorSet vk_descriptor_set::get_handle() const
{
    return handle;
}

const vk_descriptor_set_layout& vk_descriptor_set::get_layout() const
{
    return descriptor_set_layout;
}

BindingMap<VkDescriptorBufferInfo>& vk_descriptor_set::get_buffer_infos()
{
    return buffer_infos;
}

BindingMap<VkDescriptorImageInfo>& vk_descriptor_set::get_image_infos()
{
    return image_infos;
}